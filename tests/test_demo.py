...
def test_all_ungoverned_scenarios_are_labelled(
    capsys: pytest.CaptureFixture[str],
) -> None:
    """Print one solve, three breaches, and S4's ungoverned non-applicability."""
    assert main(["--mode", "ungoverned"]) == 0
    output = capsys.readouterr().out
    assert output.count("LEGITIMATE SOLVE:") == 2
    assert output.count("BREACH:") == 6
    assert "NOT APPLICABLE: ungoverned mode has no execution firewall" in output
    ...
    assert "Completed 5 ungoverned scenario(s)." in output
...
def test_scenario_selection(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert main(["--mode", "ungoverned", "--scenario", "S2"]) == 0
    output = capsys.readouterr().out
...
@pytest.mark.parametrize("arguments", [("--mode", "unknown"), ("--scenario", "S5")])
def test_future_options_are_rejected(arguments: tuple[str, str]) -> None:
    """Reject execution modes and scenarios outside the finished demo's scope."""
...
def test_governed_only(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    output = capsys.readouterr().out
    scenario_output = output.split("SUMMARY", maxsplit=1)[0]
    assert "UNGOVERNED" not in scenario_output and "C7 PERMIT" in scenario_output
    assert output.rstrip().endswith("C5 evidence: 14 records, chain OK")
